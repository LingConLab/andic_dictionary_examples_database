...
library(tidyverse)
library(DT)
# ids taken from andic dictionary db: https://github.com/phon-dicts-project/comparative_andic_dictionary_database
db <- read_csv('andic_examples.csv') %>% 
  select(-c(id_word,id_meaning,id, reference))
```


```{r}
db %>% 
  datatable(filter = 'top', rownames = TRUE,
            options = list(pageLength = 20, dom = 'tip',
                           initComplete = JS(readLines("palochka.js"))))
```
